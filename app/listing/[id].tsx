...
import { PayPalWebViewModal } from "../../components/modals/PayPalWebViewModal";
...
import { ThriftShareReport } from "../../components/modals/ThriftShareReport";
...
type State = {
    ...
    selectedPaymentMethod: 'GCash' | 'PayMaya' | 'PayPal' | 'Other / Manual Transfer' | null;
    isPaymentChoiceModalVisible: boolean;
    availablePaymentMethods: string[]; // --- NEW STATE ---
};
...
const initialState: State = {
    ...
    availablePaymentMethods: [], // --- NEW STATE ---
};
...
function reducer(state: State, action: any): State {
    switch (action.type) {
        ...
        case "SET_PAYMENT_METHODS": // --- NEW REDUCER ---
...
export default function ListingDetailScreen() {
    ...
    const fetchListingDetails = useCallback(async () => {
        ...
        // --- NEW: Fetch and set available payment methods ---
        const [paymentDetailsResult, profileResult] = await Promise.all([
            supabase
        ...
        let finalPaymentAccounts: PaymentAccount[] = [];
        let availableMethods: string[] = [];

        if (paymentDetailsResult.data?.payment_methods) {
            finalPaymentAccounts = [...paymentDetailsResult.data.payment_methods];
            paymentDetailsResult.data.payment_methods.forEach((pm: any) => availableMethods.push(pm.type));
        }

        if (profileResult.data?.paypal_email) {
            finalPaymentAccounts.push({
                id: 'paypal',
                type: 'PayPal',
                account_name: "Platform Business Account",
                account_number: profileResult.data.paypal_email,
            });
            availableMethods.push('PayPal');
        }
        
        if (data.is_cod_enabled) {
            availableMethods.push('Cash on Delivery');
        }

        setSellerPaymentAccounts(finalPaymentAccounts);
        dispatch({ type: 'SET_PAYMENT_METHODS', payload: availableMethods });
        // --- END NEW ---
    ...
    const handlePaymentAttempt = (method: 'GCash' | 'PayMaya' | 'PayPal' | 'Other / Manual Transfer') => {
    ...
    const promptForCodConfirmation = () => {
        dispatch({
            type: "SHOW_CONFIRMATION_MODAL",
            payload: {
    ...
    const sellerAccountForManualPayment = selectedPaymentMethod && selectedPaymentMethod !== 'PayPal'
        ? sellerPaymentAccounts.find(acc => acc.type === selectedPaymentMethod) || null
        : null;

    const isServiceListing = listing.categories.includes('Services');
        ...
        >
                        ...
                        {Object.entries(listing.dynamic_fields || {}).map(([key, value]) => (
                            <View style={styles.detailItem} key={key}>
                                ...
                                <View style={styles.detailTextContainer}>
                                    ...
                                    <View style={styles.categoryContainer}>
                                        {(value as string).split(",").map((item, index) => (
                                            <View
                                                key={index}
                                                style={[
                                                    styles.categoryPill,
                                ...
                                <View style={styles.detailTextContainer}>
                                    ...
                                    <View style={styles.paymentMethodsContainer}>
                                        {availablePaymentMethods.map((method, index) => (
                                            <View key={index} style={[styles.paymentMethodPill, { backgroundColor: colors.background, borderColor: colors.border }]}>
                                                <Text style={[styles.paymentMethodText, { color: colors.text }]}>{method}</Text>
                                            </View>
                                        ))}
                                    </View>
                                </View>
            ...
            {!isOwner && (
                <View style={[styles.footer, { paddingBottom: insets.bottom + 15 }]}>
                    ...
                    <View style={styles.footerActionButtons}>
                        {isServiceListing ? (
                             <TouchableOpacity
                                style={[styles.actionButton, styles.messageButton, {flex: 2}]}
            ...
            {session?.user && listing && selectedPaymentMethod && (
                <ProofOfPaymentModal
                    ...
                    sellerAccountDetails={sellerAccountForManualPayment}
                    onSubmissionSuccess={() => {
                        dispatch({ type: "TOGGLE_PROOF_OF_PAYMENT_MODAL" });
                        router.push('/(tabs)/me');
                    }}
            ...
            {session?.user && listing && (
                <PayPalWebViewModal
                    ...
                    onClose={() => {
                        ...
                        router.push({
                            pathname: '/(tabs)/me',
                            params: { openOrders: 'true', orderTab: 'buying' }
                        });
                    }}
                    amount={listing.price || 0}
                    itemName={listing.item_name}
                    onPaymentSuccess={async (paypalOrderId) => {
                        try {
                            const { error } = await supabase.from('orders').insert({
                                listing_id: listing.id,
                                buyer_id: session.user.id,
                                ...
                                reference_number: paypalOrderId,
                            });
                            ...
                            Alert.alert("Error", "Your payment was successful, but we couldn't save the order. Please contact support.");
                        }
                    }}
                />
            )}
